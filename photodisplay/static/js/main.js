			...
			titleEl.addEventListener('click', function(ev) {
				...
				if( classie.has(stack, 'is-selected') ) { // current stack
					if( classie.has(bodyEl, 'view-full') ) { // stack is opened
						...
						if( scrolled > 0 ) {
							smooth_scroll_to(isFirefox ? docElem : bodyEl || docElem, 0, 800).then(function() {
								closeStack();
							});
						}
						else {
...
function comment_show(){
	...
	document.querySelector('.comment-1').style.display="block";
	setTimeout(function(){  document.querySelector('.comment-1').style.opacity = "1";
	 },400);

}
function comment_hide(){
	
	document.querySelector('.comment-1').style.opacity="0";
	setTimeout(function(){
		document.querySelector('.comment-1').style.display = "none";
